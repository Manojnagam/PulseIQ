...
export default async function handler(req, res) {
  ...
  if (consent_given !== true) {
    return res.status(400).json({ error: 'Customer consent confirmation is required' });
  }

  if (!consent_name || typeof consent_name !== 'string' || consent_name.trim().length < 2) {
    return res.status(400).json({ error: 'Valid customer full name for consent record is required' });
  }

  const phoneDigits = String(consent_phone_last4 || '').replace(/\D/g, '');
  if (phoneDigits.length < 4) {
    return res.status(400).json({ error: 'Last 4 digits of customer phone number are required' });
  }
