...
async function callGroqVariant(groqKey, customerWords, styleHint) {
  ...
  while (attempt < 2) {
    ...
    const filterDecision = hits.length === 0
      ? 'PASSED: No prohibited terms detected.'
      : (attempt === 1
          ? `REJECTED: Prohibited term(s) [${hits.join(', ')}] detected. Triggering Attempt 2 regeneration.`
          : `REJECTED: Prohibited term(s) [${hits.join(', ')}] detected on retry. Retries exhausted.`);
