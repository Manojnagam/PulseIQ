...
function isGrowthPlan() {
  if (!isCenterSession() && !ACTIVE_CENTER) return true; 
...
function isTrialExpired() {
  if (!isCenterSession() && !ACTIVE_CENTER) return false; 
  var targetId = ACTIVE_CENTER || (_centerAuth && _centerAuth.centerId);
  var c = (D.centers||[]).find(function(x){return x.id===targetId;});
  if (!c) return false;
...
function getOrgVP(month) {
  ...
  var orgVP = 0;
  D.centers.forEach(function(c) {
    if(mainCenter && c.id === mainCenter.id) return; 
...
function getPersonalVP(month) {
  var mainCenter = D.centers.find(function(c){return c.type==='main';});
  if(!mainCenter) return calcMonthlyVP(month, null); 
...
async function loadAll() {
  ...
  var chartPromise = loadScript('https:
...
function renderOverview() {
  ...
  if(pendingPay.length){
    ...
    leftHtml += pendingPay.slice(0,8).map(function(p){
      ...
      if(p.phone) {
        ...
        waBtn = '<button class="wa-btn" style="font-size:11px;padding:3px 8px;margin-left:8px" onclick="window.open(\'https:
      }
  ...
  if(!inactiveList.length) rightHtml += '<div class="ov-empty">Everyone is active — great job!</div>';
    ...
    return '<div class="ov-row"><div class="ov-row-info"><div class="ov-row-name">'+c.name+'</div><div class="ov-row-sub">'+(lastAtt?'Last seen: '+lastAtt:'Never attended')+'</div></div><div class="ov-row-actions"><button class="wa-btn" style="font-size:11px;padding:3px 8px" onclick="window.open(\'https:
  }).join('');
    ...
    if(!riskList.length) { rightHtml += '<div class="ov-empty">All active customers are engaged!</div>'; }
      ...
      rightHtml += riskList.slice(0,8).map(function(x){
        ...
        return '<div class="ov-row"><div class="ov-row-info">'
          ...
          +(phone?'<button class="wa-btn" style="font-size:11px;padding:3px 8px" onclick="window.open(\'https:
          +'</div></div>';
      }).join('');
  ...
  if (bodyAlerts.length) {
    ...
    rightHtml += bodyAlerts.slice(0,8).map(function(a){
      ...
      return '<div class="ov-row"><div class="ov-row-info"><div class="ov-row-name">'+a.name+'</div><div class="ov-row-sub">'+a.msg+'</div></div>'
        +(phone?'<div class="ov-row-actions"><button class="wa-btn" style="font-size:11px;padding:3px 8px" onclick="window.open(\'https:
        +'</div>';
    }).join('');
    rightHtml += '</div></div>';
  }
...
function renderCurrentStock() {
  ...
  var html = products.map(function(p){
    ...
    if (p.gramsPerScoop && p.gramsPerBox && qty > 0) {
      ...
      var gramsPerShake = p.gramsPerScoop * 2; 
...
async function autoApplyRecurring() {
  ...
  if (safeStorage.getItem(lsKey)) return; 
...
async function applyRecurringNow() {
  ...
  safeStorage.removeItem('recurringApplied_' + ym); 
...
function renderAttGrid() {
  ...
  var custRows = filterByCenter(D.customers).filter(function(c){
    ...
    if((c.pack_start_date || c.created_at || c.join_date || '').startsWith(monthStr)) return true; 
    if(getDaysLeft(c).active) return true; 
...
function renderAttendance() {
  ...
  var activeCusts = filterByCenter(D.customers).filter(function(c){
    ...
    if (!packActive && !presentToday) return false; 
    if (isInactive(c) && !ATT_SHOW_INACTIVE && !presentToday) return false; 
...
function editCustomer(id) {
  ...
  document.getElementById('comp-day-section').style.display='none'; 
  ...
  (function() {
    ...
    var grid = cohortMonths.map(function(ym) {
      ...
      for (var offset = 0; offset < maxForward; offset++) {
        ...
        var now = new Date().toISOString().substring(0, 7);
        if (checkYm > now) { row.push(null); continue; } 
...
async function saveRenewal(){
  ...
  try{
    ...
    if(c&&c.contact){
      ...
      var phone=c.contact.replace(/\D/g,'');if(phone.length===10)phone=COUNTRY_CODE+phone;
      setTimeout(function(){if(confirm('Send WhatsApp renewal confirmation to '+displayName+'?'))window.open('https:
    }
  }catch(e){showToast('Error: '+e.message,'error');}
}
...
function renderBizAnalyst() {
  ...
  coachData.forEach(function(d, idx) {
    ...
    html += '<div class="tcard" style="padding:18px;border-top:3px solid '+rankColor+'">'
      ...
      +(phone?'<button class="wa-btn" style="font-size:11px;padding:4px 10px" onclick="window.open(\'https:
      +'</div>'
      +'</div>';
  });
  html += '</div>';
...
async function saveShakeRedemption(){
  ...
  try{
    ...
    if(phone){
      ...
      var bar=document.getElementById('att-wa-bar');
      if(waLink&&bar){waLink.href='https:
      else{window.open('https:
    }
  }catch(e){showToast('Error: '+e.message,'error');}
}
...
function renderWalkins() {
  ...
  tb.innerHTML = rows.slice(0, window._limWalk).map(function(w) {
    ...
    var waBtn = w.phone ? '<a class="wa-btn" href="https:
...
function renderLeads() {
  ...
  leads.forEach(function(lead) {
    ...
    var waBtn = lead.mobile ?
      '<button class="btn-e" style="font-size:11px;padding:5px 10px;background:#25D366;color:#fff;border:none;border-radius:6px;cursor:pointer" onclick="window.open(\'https:
  ...
  data.forEach(function(d, i) {
    ...
    html += '<tr>'
      ...
      +'<td style="font-weight:700;color:var(--primary)"><div style="display:flex;align-items:center;gap:6px">₹'+d.total.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})
        +(phone&&d.total>0?' <button class="wa-btn" style="font-size:10px;padding:2px 7px" onclick="window.open(\'https:
      +'</div></td>'
      +'</tr>';
  });
  ...
  list.forEach(function(person) {
    ...
    if (!person.isOwner) {
      ...
      if (phone) {
        ...
        actionsHtml = '<button class="wa-btn" style="font-size:11px;padding:4px 8px" onclick="window.open(\'https:
      } else {
...
async function toggleSentToSenior(contestId) {
  var c = D.contests.find(function(x){ return x.id===contestId; });
  if (!c) return;
  if (c.amount_sent_to_senior) return; 
...
function openQrKioskModal() {
  ...
  document.getElementById('qr-kiosk-img').src = 'https:
  openModal('qr-kiosk');
}
